...
///
/// The flag is trustworthy on its own as of Wolverine 6.26.0. Before it, a chain whose only Marten shape was
/// <c>[WriteAggregate]</c> got a commit frame while <c>chain.IsTransactional</c> stayed false, so this policy
/// carried a second check for that shape — JasperFx/wolverine#3893, fixed by JasperFx/wolverine#3901. 6.27.0
/// (#3911) closed the same disagreement for a chain made transactional by an <c>IMartenOp</c> return.
///
